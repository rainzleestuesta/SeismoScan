...
export default function App() {
  ...
  function onMapReady(map:any){
    mapRef.current = map;
    map.on("moveend", ()=> {
      const c = map.getCenter(); setCenter({lat: c.lat, lon: c.lng});
    });
    ...
    loadFaultsGeoJSON(FAULTS_URL).then(gj=>{
      ...
      map.addLayer({ id:"faults-line", type:"line", source:"faults-src",
    ...
    fetchOpenAreasNCR().then(list=>{
      ...
      map.addLayer({ id:"safe-labels", type:"symbol", source:"safe-src", layout:{ "text-field":["get","name"], "text-size":11, "text-offset":[0,1.2], "text-anchor":"top" }, paint:{ "text-color":"#065f46", "text-halo-color":"#fff", "text-halo-width":1 }});
    });
  }
  ...
  async function navigateToNearest() {
    ...
    if (geom && mapRef.current) {
      ...
      mapRef.current.fitBounds(coords.reduce(([minX,minY,maxX,maxY],[x,y])=>[
        Math.min(minX,x), Math.min(minY,y), Math.max(maxX,x), Math.max(maxY,y)
      ], [coords[0][0], coords[0][1], coords[0][0], coords[0][1]] as any), { padding: 64, duration: 900, pitch: 35, bearing: br});
    }
  }
